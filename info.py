...
BOT_TOKEN = environ.get('BOT_TOKEN', '7789146237:AAEjSosWD9Ym3VXIKJP7jeQR7N7B8fo-vLI')
...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002465245077').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '6195556018').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
auth_channel = environ.get('-1002360021106')
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None

# MongoDB information
DATABASE_URL = environ.get('DATABASE_URL', "mongodb+srv://Prahud232:prahud232@prahud232.mzgal.mongodb.net/?retryWrites=true&w=majority&appName=Prahud232")
DATABASE_NAME = environ.get('DATABASE_NAME', "prahud232")
